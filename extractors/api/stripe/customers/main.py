...
def fetch_stripe_customers(days_from: int = DAYS_FROM) -> list:

    """
    Fetches Stripe customers created after the specified number of days ago.
    
    Args:
        days_from: Number of days ago to start fetching from (default: from env var DAYS_FROM)
                  If days_from=1, fetches customers created from yesterday onwards
    ...
    logger.info(f"Fetching Stripe customers created after {start_date.date()} (timestamp: {start_timestamp})")
        ...
        if payload:
            ...
            for customer in payload:
                ...
                if customer_id and customer_id not in seen_ids:
                    ...
                    unique_payload.append(customer)
                elif customer_id:
                    logger.debug(f"Skipping duplicate customer: {customer_id}")
            
            if unique_payload:
                raw_data.append(unique_payload)
                logger.info(f"Fetched page with {len(unique_payload)} unique customers (skipped {len(payload) - len(unique_payload)} duplicates)")
        
        next_page_id = customers.next_page
        if not next_page_id:
            break
    
    total_customers = sum(len(page) for page in raw_data)
    logger.info(f"Total unique customers fetched: {total_customers} across {len(raw_data)} pages")
...
def transform_to_bigquery_table(raw_data: list) -> pd.DataFrame:
    ...
    logger.info(f"Processing {len(all_customers)} customers")
    ...
    for customer in all_customers:
        ...
        if hasattr(customer, '__dict__'):
            ...
            logger.warning(f"Skipping customer with unexpected type: {type(customer)}")
            continue
        
        # Extract ID
        customer_id = customer_dict.get('id')
        if not customer_id:
            logger.warning("Skipping customer without ID")
            continue
            
        created_at = customer_dict.get('created')
        if not created_at:
            logger.warning("Skipping customer without created_at")
            continue
        
        # Parse Unix timestamp (seconds) to date object (yyyy-mm-dd format)
        created_at = datetime.datetime.fromtimestamp(created_at, tz=datetime.timezone.utc).date()
        
        # Convert entire customer to JSON string
        data_json = json.dumps(customer_dict, default=str, ensure_ascii=False)
...
def merge_to_bigquery(
    ...
    # Define schema for the 4 columns
    schema = [
...
if __name__ == "__main__":
    ...
    for i in range(DAYS_FROM, DAYS_TO - 1, -1):
        raw_data = fetch_stripe_customers(i)
