...
def fetch_stripe_subscriptions(days_from: int = DAYS_FROM) -> list:

    """
    Fetches Stripe subscriptions created after the specified number of days ago.
    
    Args:
        days_from: Number of days ago to start fetching from (default: from env var DAYS_FROM)
                  If days_from=1, fetches subscriptions created from yesterday onwards
    ...
    logger.info(f"Fetching Stripe subscriptions created after {start_date.date()} (timestamp: {start_timestamp})")
        ...
        if payload:
            ...
            for sub in payload:
                ...
                if sub_id and sub_id not in seen_ids:
                    seen_ids.add(sub_id)
                    unique_payload.append(sub)
                elif sub_id:
                    logger.debug(f"Skipping duplicate subscription: {sub_id}")
            
            if unique_payload:
                raw_data.append(unique_payload)
                logger.info(f"Fetched page with {len(unique_payload)} unique subscriptions (skipped {len(payload) - len(unique_payload)} duplicates)")
        
        next_page_id = subscriptions.next_page
        if not next_page_id:
            break
    
    total_subscriptions = sum(len(page) for page in raw_data)
    logger.info(f"Total unique subscriptions fetched: {total_subscriptions} across {len(raw_data)} pages")
...
def transform_to_bigquery_table(raw_data: list) -> pd.DataFrame:
    ...
    for page in raw_data:
        if page:
            all_subscriptions.extend(page)
    
    logger.info(f"Processing {len(all_subscriptions)} subscriptions")
    ...
    for sub in all_subscriptions:
        ...
        if hasattr(sub, '__dict__'):
            ...
            logger.warning(f"Skipping subscription with unexpected type: {type(sub)}")
            continue
...
if __name__ == "__main__":
    ...
    for i in range(DAYS_FROM, DAYS_TO - 1, -1):
        raw_data = fetch_stripe_subscriptions(i)
